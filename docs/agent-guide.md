...
# Backend
cd backend
dotnet restore FCI.sln
dotnet build FCI.sln
dotnet test FCI.sln
...
# Database (one-shot; assumes local Postgres 16 on :5432 with db `fci`)
psql -U postgres -d fci -f ../backend/FCI.Data/Migrations/V001__initial_schema.sql
```

Copy `frontend/.env.template` → `frontend/.env.dev` and `backend/FCI.Api/appsettings.template.json`
→ `backend/FCI.Api/appsettings.Development.json`. Fill in the placeholder values from your
...
3. Tests alongside implementation. ≥85 % coverage on `FCI.Engine` and `FCI.AI`.
...
### Add a new EF entity
1. Create `backend/FCI.Data/Entities/{Name}.cs`.
2. Add `DbSet<{Name}> {Names}` to `FciDbContext`.
3. Add a new SQL migration `V0NN__add_{name}.sql`.
4. Add a soft-delete query filter and tenant filter in `OnModelCreating`.
5. Add a repository or store class in `FCI.Api/Services/`.

### Add a new API endpoint
1. Add the action to the relevant controller (or create a new one).
2. Add request/response DTOs to `backend/FCI.Api/Dtos/`.
3. Mirror to `frontend/src/models/` and `frontend/src/api/{domain}Client.ts`.
4. Add controller-level tests in `backend/FCI.Tests/FCI.Api.Tests/Controllers/`.
...
4. Add a tests fixture to `backend/FCI.Tests/Fixtures/quality/`.
5. Document the rule in `docs/spec.md` §9.4 (in a follow-up doc PR).

## 6. Workload publishing runbook (Sprint 6 deliverable)

1. Build the manifest: `npm run build:manifest` produces
   `frontend/dist/manifest/Datachain.FabricContractIntelligence.{version}.nupkg`.
...
4. Upload the `.nupkg`. Verify item types appear: `Contract`, `ContractPolicy`,
   `ContractReport`.
5. Assign the workload to a target capacity for a private-preview workspace.
6. Smoke test: open the workspace → New Item → choose `Contract` → editor renders.
...
unsure, **say so** — leave a comment in the PR description and ping `@datachain/fci-core`.

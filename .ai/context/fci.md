...
This file defines the vocabulary of the FCI product. Agents should use these terms exactly —
...
| Term | Meaning |
|---|---|
| **Tenant** | A Microsoft Fabric / Entra tenant that has installed FCI. One row in `tenants`. |
...
- **OneLake Delta tables** are stored as Parquet + a `_delta_log/` directory of JSON
  transaction commits. FCI reads only the **transaction log** for schema/freshness; quality
...
- FCI is exclusively a **contract definition + enforcement** layer.
